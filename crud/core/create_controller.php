...
$string .= "\n\t    );
    ...
    public function update(\$id) 
    {
        ...
        if (\$row) {
            \$data = array(
                'pagename' => 'Kemaskini Borang ".ucfirst(label($table_name))."',
                'button' => 'Update ".ucfirst(label($table_name))."',
...
if ($export_excel == '1') {
    $string .= "\n\n    public function excel()
    {
        ...
        \$columnHead = 0;
        xlsWriteLabel(\$tablehead, \$columnHead++, \"No\");";
foreach ($non_pk as $row) {
        $column_name = label($row['column_name']);
        $string .= "\n\t\txlsWriteLabel(\$tablehead, \$columnHead++, \"$column_name\");";
}
$string .= "\n\n\tforeach (\$this->" . $m . "->get_all() as \$data) {
            \$kolombody = 0;

            //change xlsWriteLabel became xlsWriteNumber for numeric column
